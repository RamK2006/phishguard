...
from app.db.models import ScanEvents, RiskLevel, ScanSource
...
async def scan_url(
    ...
    scan_event = ScanEvents(
        ...
        features=features,
        virustotal_result=cti_result.get("sources", [{}])[0] if cti_result.get("sources") else None,
        explanation=explanation,
        source=ScanSource.EXTENSION if req.source == "extension" else ScanSource.API,
        scan_type="url",
...
async def scan_email(
    ...
    for link in req.links[:50]:  # Max 50 links per email
        try:
            ...
            result = await scan_url(scan_req, request, db)
